...
def prepare_out_blend_frame(blend_on_road, img_binary, img_birdeye, img_fit, line_lt, line_rt, offset_meter):
    ...
    if line_lt.detected:
        ...
        cv2.putText(blend_on_road, 'Curvature radius: {:.02f}m'.format(mean_curvature_meter), (860, 10), font, 0.9,
                    (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(blend_on_road, 'Offset from center: {:.02f}m'.format(offset_meter), (860, 70), font, 0.9,
                    (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(blend_on_road, 'Detected lef and right: {}'.format(str(line_lt.detected and line_rt.detected)),
                    (860, 130), font, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
    else:
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(blend_on_road, 'UNDETECTED LINES', (860, 10), font, 0.9,
                    (255, 255, 255), 2, cv2.LINE_AA)
...
def process_pipeline(frame, keep_state=True):
    ...
    blend_on_road, intensity_lt, intensity_rt, intensity_road = draw_back_onto_the_road(img_undistorted, Minv, line_lt,
                                                                                        line_rt, keep_state)
    csv.write("{}, {:.3f}, {:.3f}, {:.3f}, {}\n".format(processed_frames, intensity_lt, intensity_rt, intensity_road, str(line_rt.detected)))
    csv.flush()
...
if __name__ == '__main__':
    ...
    if mode == 'video':
        selector = 'scaledvideo720'
        csv = open("out_{}_{}.csv".format(selector, time_window), "w")
        csv.write("frame_number, left_line_intensity, right_line_intensity, road_line_intensity, detected\n")
        clip = VideoFileClip('{}.mp4'.format(selector)).fl_image(process_pipeline)
